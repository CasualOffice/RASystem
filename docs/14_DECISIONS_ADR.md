...
- **ADR-103 · 1:1 voice/video calling: expand the media posture from output-only to two-way (mic + camera capture), consent-gated and never recorded · Accepted (2026-07, owner sign-off).**
  ...
  - **Decision — reuse the pipeline in both directions; add only two new capture backends.**
    ...
    - **Camera video, both directions:** reuse the existing **encode → per-frame QUIC uni-stream → decode** video path (`VideoEncoderBackend`, `PerFrameStream`, ADR-060) and the WebCodecs decode/render — camera frames are just another `EncodedFrame` source. **No new video transport, encoder, or wire frame type.**
    - **Genuinely new (the only new native surface):** a **microphone capture backend** (`AudioCaptureBackend` — the trait already exists; only the OS-mic implementations are new) and a **camera capture backend** (a new `CameraCaptureBackend` trait mirroring `ScreenCaptureBackend`, producing the same `CapturedFrame`/`SurfaceKind::CpuBgra` the encoder already consumes), each per-OS: macOS (AVFoundation), Windows (Media Foundation / MediaCapture), Linux (PipeWire camera + PulseAudio/PipeWire mic). New crates `ras-mic-<os>` / `ras-camera-<os>` with `unsafe` confined to the FFI, empty off-platform (the established backend pattern). No change to the encoder/decoder crates, the datagram/uni-stream transport, or the orchestrator wire.
  - **Inv 12 preserved in spirit (the load-bearing part).** The core of Inv 12 — **live-only, never recorded, no media at rest** — is **retained unchanged and extended, not relaxed**: (1) call media is **live-only** — mic PCM and camera frames flow encode→wire→decode→play/render and are **never written to disk, never buffered for replay, never persisted**; there is **no recording path and none is added** (a compile-time absence, matching the fraud subsystem's forbidden-`content`-field posture, Inv 11). (2) The one thing that genuinely changes vs. today is **directionality + source** (output-only → two-way; screen → also mic + camera), and it is made safe by **explicit consent**: a call is **initiated and accepted** (docs/25 §7 Ring→Accept), so mic and camera each turn on only inside an accepted, live call — never ambiently. (3) **Always disclosed, unspoofably (Inv 7):** mic and camera each get their **own always-visible active-state indicator** (distinct from "SCREEN SHARED"/"AUDIO SHARED"), backed by the secure-window capture-exclusion so the indicator can't be re-captured/spoofed; turning a source off removes its indicator. The Stop/End control remains always present and overriding (Inv 4).
  - **Capability model (Inv 2/15).** Two **new named capabilities** — `audio.mic.capture` and `video.camera.capture` — added to the registry (docs/04) as **recognized-but-withheld, deny-by-default**, exactly like `audio.listen`/`clipboard.*`. They are **enforced per-message/per-session host-side** through the same authorization path (never trust the peer's claimed scope, RustDesk-CVE class): mic frames without `audio.mic.capture` and camera frames without `video.camera.capture` are dropped at the gate. Because a call is symmetric, **each side** grants these for **its own** mic/camera — a peer can never enable your capture; consent to be *in* a call is not consent to have your mic/camera on, so the two capabilities are togglable independently mid-call (docs/25 §7 mute/camera-off state-fill). They are **not** in the default-grantable policy; they never widen the *remote-access* grant/lease (calling and OS control are orthogonal surfaces).
  - **Fraud/harm-subsystem interaction (Inv 11/12).** The fraud analyzer is a pure on-device `content → verdict` function and must have **zero content at rest**. Call media (mic PCM, camera pixels) is **content** in that sense, so the rule is explicit: **call media never enters the fraud subsystem and never creates content-at-rest** anywhere. No screenshot/keystroke/recording buffer is introduced; the analyzer sees only content-free verdict enums, unchanged by this ADR. Camera/mic being live-only + never-persisted is exactly what keeps Inv 11's "content never crosses a boundary / no content at rest" intact.
  - **Priorities (§2).** Security first (deny-by-default caps, per-message gate, unspoofable indicators, no recording path); latency second (reuse the datagram/per-frame-stream paths already tuned for it; a stalled camera feed must never freeze audio, the pointer, or End); UX third. No inversion of the ordering; if calling ever needed to record, that is a *separate* ADR reversing Inv 12's core, not this one.
  - **Status.** **Proposed (pending sign-off).** Not yet Accepted — this expands the media posture and needs the owner's approval before any code. Depends on the call-signaling ADR (docs/25 §12) landing alongside. Considered + deferred: group calling (1:1 only for now); mic/camera *recording* (explicitly out — would reverse Inv 12's core); an in-app camera/mic *preview* persistence (none — preview is live-only like the call).

---

IMPLEMENTATION NOTES (for the plan):
- **Reused unchanged:** `ras-media::audio` traits + `AudioConfig`/`EncodedAudio`; `ras-audio-opus` (both directions); the Opus-over-iroh-datagram transport (ADR-077, run symmetrically); the `VideoEncoderBackend` + `PerFrameStream` uni-stream video path + WebCodecs decode (ADR-060) for camera frames both ways.
- **New native backends (the only new capture surface):** `ras-mic-{macos,windows,linux}` implementing the existing `AudioCaptureBackend`; `ras-camera-{macos,windows,linux}` implementing a new `CameraCaptureBackend` trait (mirrors `ScreenCaptureBackend`, emits `SurfaceKind::CpuBgra`). `unsafe` confined to FFI, empty off-platform.
- **New non-backend surface:** two registry capabilities `audio.mic.capture` + `video.camera.capture` (deny-by-default, per-message gated); two new always-visible mic/camera active indicators (Inv 7); symmetric two-way wiring in the orchestrator (`ras-core`).
- **Explicitly out of scope here:** call signaling/state machine (separate ADR), group calls, and any recording/persistence path (would reverse Inv 12's core).

- **ADR-104 · 1:1 call signaling — out-of-session ring signal + in-session control plane + the call state machine · Proposed (pending sign-off).**
  - **Context.** Docs/25 §7/§12 direct Casual RAS to add 1:1 voice/video calls with an explicit,
    Zoom/Telegram-grade state model and CallKit-grade ring reliability. Calling needs two distinct
    things the existing surfaces don't yet cover: a way to **ring a contact who has no live session with
    us** (place the call, get accept/reject/busy/no-answer *before* any media path exists), and a way to
    carry **in-call control** (mute-state changes, hang-up) once a media session is up. Ringing is
    logically identical to `AccessRequestIntent` — a signed, contacts-only, out-of-session solicitation
    that authorizes nothing and raises a local surface — so it belongs on the same signaling path; in-call
    control is a session-phase concern that belongs on the established control channel. Depends on ADR-102
    (signal replay dedup) and ADR-103 (mic/camera capabilities); paired with docs/25 §7's state enum.
  - **Decision — where signaling lives (the split).** (1) The **call INVITE (ring)** is a **new signed
    `SignalPayload::CallInvite { issued_at, media: CallMediaKind }`** riding the existing `SIGNAL_ALPN`
    (`casual-ras/signal/1`), reusing `encode_signed`/`verify_signed` verbatim — Ed25519-authenticated,
    **contacts-only deny-by-default**, freshness-bounded, and deduped by `SignalReplayGuard` so a captured
    INVITE can't re-ring (ADR-102). It is a **peer** of `AccessRequestIntent`: a small tag + timestamp +
    an audio/video enum, no capabilities, no body. **Cancel-ring** (caller hangs up during ring, before any
    session) is a second signal variant `CallCancel`, since there is still no session channel to carry it.
    (2) Every **post-establishment** control message — `CallAccept`, `CallReject{code}`, `CallBusy`,
    `CallHangup{code}`, `CallMuteState{audio_muted, video_muted}` — is a **new `ControlMsg` variant** on the
    already-authenticated session control channel, alongside `ChatMessage`/`ControlRequest`. Justification:
    an INVITE must reach a peer with **no session to us yet** (only the signal plane spans that gap, and it
    is exactly the authenticated-contact, out-of-session, self-signing plane), whereas once the callee
    accepts and the media session is dialed + grant-validated, both peers share a live, endpoint-bound,
    ordered control channel that is the correct — and cheaper, no re-sign-per-message — home for in-call
    control. Acceptance is what *triggers* the session dial; before that there is no `ControlMsg` transport
    to use. All new variants get bounded, fail-closed codecs (unknown tag → drop) matching the house pattern.
  - **Decision — the state machine.** A **total, fail-closed, terminal-once** call FSM mirroring the session
    state machine's rigor, **one active call at a time** (a second inbound INVITE while non-`Idle` is answered
    `CallBusy` and never rings the user). Live states: `Idle → Ringing → Accepted → Connecting →
    Authorizing("exchanging keys") → Connected → Reconnecting → Ended`, where **`Authorizing` maps onto the
    existing grant-validation / consent step** (Inv 1/9) rather than being a cosmetic label, and
    `Reconnecting` reuses the ADR-091 session resume path. Terminal states: `Declined` (callee rejected),
    `Busy` (callee already in a call), `NoAnswer` (ring timeout, ~30–45 s, ≤ CallKit's 60 s cap), `Failed`
    (transport/handshake error), `Cancelled` (caller withdrew during ring). Every terminal state is
    absorbing; every non-terminal state has a defined edge for each event and an unexpected event fails
    **closed** to `Ended`/`Failed`, never into an ambiguous live state. The FSM is a **pure** (clock-injected)
    module — no I/O, deterministic, unit- and property-testable off-device exactly like the session FSM.
  - **Security.** A `CallInvite` **authorizes nothing** (Inv 9): it is an intent, identical in authority to
    `AccessRequestIntent`. **Accepting is the local user's explicit consent** (Inv 1) — the callee's `Accept`
    is what initiates the session dial; a ringing phone grants no access by itself. **Mic and camera are
    separately capability-gated per-message host-side** (ADR-103, Inv 15) — a `CallAccept` never confers
    media; each captured mic/camera frame is authorized against the live grant at the moment of send, so
    "in a call" ≠ "may be heard/seen," and revoking a media capability mid-call silences that stream without
    ending the call. The ring is **replay-protected** (ADR-102) and **rate-limited** by the inbound-signal
    concurrency cap plus the one-active-call rule (a flood is answered `Busy`, not surfaced). All
    lifecycle/audit events are **content-free** (Inv 8/10 — tags + counters, never a callee label, ringtone,
...
- **ADR-105 · Contact-requests-access flow: a signaling+UX layer over the unchanged authorization core · Proposed (pending sign-off).**
  ...
  - **Decision (1) — the wire: a new signed `SignalPayload` variant, distinct from `AccessRequestIntent`.** Add `SignalPayload::AccessRequest { issued_at, capabilities, reason }` (working name; a fourth `TAG_ACCESS_REQUEST` in `ras-signal`) carrying **(a)** the **requested capability set** the contact is asking for (bounded, canonical `put_str`-style count-prefixed strings, `MAX_SIGNAL_CAPS`/`MAX_SIGNAL_CAP_LEN` mirroring `ras-grant`'s `MAX_CAPS`/`MAX_CAP_LEN`) and **(b)** a short human `reason` (reuse `MAX_SIGNAL_REASON`). It rides the **existing** `casual-ras/signal/1` ALPN and inherits, **unchanged**, every property of `ras-signal`: Ed25519 sign-then-verify (`encode_signed`/`verify_signed`), **contacts-only deny-by-default** authorization (a stranger/blocked peer is dropped before decode), freshness bounding, and **replay protection via the on-wire signature + `SignalReplayGuard`** (ADR-102 — a captured request can't be re-injected to re-nag). It is deliberately **separate** from the existing `AccessRequestIntent` because that variant is the *inverse* nudge ("I'll share **my** screen with you" / "call me") and carries no requested-capability set; conflating them would overload one tag with two opposite directions. The requested caps are **not authorization input** — they are a *pre-fill hint* for the consent card, trusted only after the signature verifies and re-intersected at issuance like any other requested set (Inv 9).
  - **Decision (2) — the consent card is a DECISION surface, not a chat message** (§8 Flow 2, AnyDesk editable-checklist model). On a verified inbound `AccessRequest` signal the app raises the same focus-raising, notify-on-inbound consent card the ticket dial raises (never a chat bubble — a request is a decision). It shows **the requester's contact identity + their Crockford-base32 `pairing_code`** for the human key-check (Inv 16), the `reason`, and the requested capabilities as an **itemized, editable per-capability checklist**. Rules, structural: **deny-by-default**; **unknown/unrecognized caps are never pre-enabled** (Inv 2 — the card seeds its checkboxes from `recognize(requested) ∩ owner_policy`, so an unknown cap is absent, not defaulted-on); the owner **can only narrow, never widen** — the accept path computes `consented = requested ∩ owner_checklist_choice`, and since issuance already applies `requested ∩ policy ∩ consented`, an owner-added capability the contact didn't request (or policy doesn't allow) simply cannot appear in the grant. Allow / Deny; Deny or timeout fails closed (Inv 1).
  - **Decision (3) — "Remember this contact / skip the prompt" reuses `PairingRegistry` + `unattended_decision`, governing ONLY the prompt** (ADR-084/085). "Remember this contact (skip future prompt)" pairs the requester in the `PairingRegistry` (ADR-084) and, on a subsequent request, `unattended_decision(is_paired, tier, authorization, now)` decides whether to **skip the live card** — *nothing more*. The structural rule from those ADRs is preserved verbatim: a `Proceed` **issues nothing**; every connect still mints + validates a fresh, endpoint-bound, per-message-enforced, emergency-stoppable `SessionGrant`, and the tier cap still applies (software-only Tier 0 can never skip the prompt, Inv 16). **The UI must say this plainly** on the toggle ("Skips the approval prompt for this contact. Every session is still a fresh, scoped, revocable grant." — §8). Revocation = de-list the contact **or** drop the standing authorization; either falls back to attended consent.
  - **Decision (4) — a ticket-dial and a contact-request converge on ONE consent/grant path** (Inv 9). Both Flow 1 (ticket) and Flow 2 (contact request) terminate at the **same** consent card → **same** `AccessRequest`/`validate`/`issue` pipeline. A ticket **authenticates identity, not authority**; a contact request likewise only authenticates the requester (contacts-only) and *pre-fills* the card. Neither auto-connects; neither can self-authorize. The only difference between the two flows is *how the card gets pre-populated and whether a saved-contact identity/pairing shortcut is offered* — never *whether* consent + a fresh scoped grant happen.
  - **Trade-off / posture.** New wire surface (one signed signal variant) is why this is an ADR, but the security surface is small: it is a peer of `AccessRequestIntent`, reuses `ras-signal`'s entire verify/authorize/replay spine, and touches **no** capability, grant, lease, or invariant — it *strengthens* the initiation UX without moving the authorization boundary. Considered + rejected: (a) **overloading `AccessRequestIntent`** with an optional capability list — conflates two opposite directions on one tag and risks a UI treating an inbound "I'll share with you" as "give me control"; (b) **letting a `Remember`-ed contact skip straight to a session with no grant** — rejected outright, it would break the ADR-084/085 structural rule and Inv 1/3/15.
  - **Status.** **Proposed (pending sign-off).** Blocks on: the `ras-signal` variant + its canonical encode/decode + fuzz/round-trip tests (the pure, off-device-testable core), then the consent-card pre-fill wiring. No `ras-grant` change. Aligns with `docs/25 §8` Flow 2 and `docs/25 §12` ("Contact-request access flow — one new signal variant + the consent-card UX; the authorization core is unchanged — Short ADR").

---
  - **Implementation (notes for the plan).** New wire: exactly one `SignalPayload` variant — `AccessRequest { issued_at, capabilities: CapabilitySet, reason: String }` — with a fourth canonical `TAG_ACCESS_REQUEST` in `ras-signal`, count-prefixed caps bounded by new `MAX_SIGNAL_CAPS`/`MAX_SIGNAL_CAP_LEN` (mirror `ras-grant`'s `MAX_CAPS`/`MAX_CAP_LEN`), reusing `MAX_SIGNAL_REASON`; extend `encode_canonical`/`decode_canonical`/`check_sizes`/`Debug` (reason is fine to show, no `Redacted` needed — it's UI text, not a secret). Reused unchanged: `encode_signed`/`verify_signed` (sign→verify→contacts-only→freshness), `SignalReplayGuard` + `replay_tag` (ADR-102 replay defense), and the **entire** `ras-grant` pipeline — `AccessRequest::signed`, `validate_access_request`, `LocalHostGrantIssuer::issue` with `requested ∩ policy ∩ consented`, plus `PairingRegistry` (ADR-084) and `unattended_decision` (ADR-085) for the "Remember" shortcut. App-layer only beyond the variant: pre-fill the existing consent card from `recognize(requested) ∩ policy` (deny-by-default checklist, owner-narrows-only), and route a verified inbound signal into the *same* consent→request→grant path a ticket dial uses. No new authorization primitive; `ras-grant` is untouched.

- **ADR-106 · Symmetric 1:1 call transport — a bidirectional media session over a dedicated call ALPN, reusing the existing session planes both ways · Proposed (pending sign-off).**
  - **Context — the blocker this resolves.** The calling stack is built end-to-end *except* the transport a live call rides: the pure FSM/manager/runtime (ADR-104, `ras-call`), both wire planes (the `CallInvite` ring signal + the in-session `ControlMsg::Call*` set), the mic/camera capture backends (`ras-mic`/`ras-camera`), and the `ras-core::call::CallDriver` with its four seams (control/signal/media/events) all exist and are tested. But the driver's `CallControlSink` and `CallMediaController` need a *connection* to write to, and today there is none for a call. The **ring** already works over the existing `casual-ras/signal/1` ALPN (out-of-session, signed, contacts-only). What is missing is the **in-session media path**: the screen-session transport (`casual-ras/1`, `IrohSessionTransport`) is deliberately **one-way** — the host *serves* video/audio and the controller *consumes* — whereas a call is **symmetric** (both peers capture + send *and* receive + play). This is an architecture decision, not glue, so it gets an ADR before the app wiring (L6a) can be a working call.
  - **Decision (1) — a dedicated call connection + ALPN, orthogonal to screen sessions.** On accept (the callee's `CallAccept`), the two peers establish a **call session** over a **new ALPN `casual-ras/call/1`**, dialed peer-to-peer by saved `ContactId` through the existing endpoint/bootstrap dial (contacts-only, endpoint-authenticated exactly like the signal plane — a stranger/blocked peer is refused). It is **distinct** from the screen-session ALPN so a call and a screen-share session are fully independent connections: **being in a call never implies screen view or OS control, and vice-versa** (the orthogonality promise in `docs/design/phase-call-design.md §1`).
  - **Decision (2) — reuse the media primitives, run symmetrically (no new codec/wire/frame type).** Each peer runs **both** an egress and an ingress of the *already-built, already-tested* transport primitives: **Opus over unreliable QUIC datagrams** (ADR-077) each way for mic audio, **per-frame QUIC uni-streams** (ADR-060) each way for camera video, and **one bidi control channel** for the `ControlMsg::Call*` messages. A **`CallTransport` trait** (mirroring `SessionTransport`) exposes `control_channel()` + an audio egress/ingress pair + a video egress/ingress pair; the iroh impl assembles them from the existing `PerFrameStream`/audio-datagram/`FramedControlChannel` building blocks, just wired in **both** directions. The load-bearing point: **the only genuinely new thing is the bidirectional orchestration + the call ALPN** — there is no new codec, frame type, or authorization primitive.
  - **Decision (3) — the `CallDriver` seams map straight onto it.** `CallControlSink` → the call control channel; `CallMediaController::start(media)` → spin this side's mic (+ camera iff video) **capture → encode → egress** pumps *and* attach the peer's **ingress → decode → play/render**; `stop()` tears both down. `CallSignalSink` stays on the **signal** ALPN (the ring is out-of-session — unchanged). `CallEventSink` → the app. So L6a becomes: implement `CallTransport` (iroh) + these four app seams over it, and the `CallDriver` (already tested) drives it.
  - **Security (unchanged posture, restated for the new plane).** The call connection is **endpoint-authenticated + contacts-only** (dial by saved `ContactId`; refused at accept otherwise — same as signal). Mic/camera capture is **still per-message capability-gated at the egress boundary** (`audio.mic.capture` / `video.camera.capture`, Inv 15) — a peer can never enable your capture, and muting = *ceasing to send* that stream. **Emergency stop / hangup tears capture down synchronously before the next send** (Inv 4). **Live-only, never recorded** — encode→wire→decode→play, no disk, no replay buffer (Inv 12). Audit is content-free (Inv 8/10). Transport authenticates identity, **never authority** (Inv 9) — being connected for a call grants nothing beyond the call.
  - **Trade-off / considered + rejected.** New surface = one ALPN + a symmetric transport assembly + a `CallTransport` seam, but built **entirely from existing tested primitives run both ways**, so the new-code risk is orchestration, not protocol. Considered + rejected: (a) **reusing the screen-session `casual-ras/1` connection for call media** — conflates two orthogonal surfaces (a call would ride a screen grant, breaking the "call ≠ screen access" promise) and the session transport is structurally one-way; (b) **a WebRTC data/media channel for the call** — deferred to the browser-controller SDK track (ADR-057), not the native P2P path; (c) **a single shared bidi stream for both peers' audio** — rejected, HOL-blocking + no per-direction loss isolation (datagrams each way is the ADR-077 rationale, symmetric).
  - **Status. Proposed (pending sign-off).** **Blocks L6a** (the app call glue cannot be a *working* call until this lands; the ring/UI/driver already work). Partly off-device-testable — a `CallTransport` in-memory loopback (mirroring the session `testkit`) can drive two `CallDriver`s through a full call locally; the real two-endpoint iroh call + live mic/camera is on-device. Depends on ADR-103/104 (Accepted) + the existing ADR-060/077 media transport.


- **ADR-107 · Annotation extracted as an embeddable SDK; Jitsi first, over the bridge relay, rendered on the sharer's real desktop · Proposed (pending sign-off).**
  - **Context.** The annotation slice proven in `v0.0.4-alpha` (ADR-097 wire+core+overlay, ADR-100 the transparent click-through overlay discipline that made it actually usable) is the first piece of this repo worth extracting as the SDK the whole product thesis promises (`docs/05`, M7). The first integration target is **Jitsi** — a multi-party meeting product we do not own, with its own transport, its own layout engine, and browser participants. Two things about that target are new relative to RAS: there are **N annotators, not one**, and the data path must work for **browser** participants, not just our native app. On-device trial in a live Jitsi meeting confirmed the behaviour worth keeping and the gap: marks drawn on the sharer's screen were visible to *every* participant with no receive-side code, but there were no per-participant cursors.
  - **Decision (1) — keep the RAS model: render on the sharer's real desktop, not on a video element.** A transparent, always-on-top, click-through window on the screen-sharer's machine is the **sole renderer**. Because those pixels are inside the screen capture, the marks reach every participant — browser included — over the ordinary video path. This is the load-bearing consequence and it collapses the hard part of multi-party: **one renderer, no replicated state, no CRDT, no late-joiner replay**, and a lossy/unordered relay is sufficient. ADR-100's rule is carried verbatim: the overlay is **always transparent + click-through and never made interactive** (making it opaque/interactive is what produced the macOS white screen and hidden context menus).
  - **Decision (2) — transport is a plug-in; Jitsi's bridge relay is the first adapter.** Ops ride the JVB endpoint-message relay — `conference.sendMessage(payload, to, /* viaBridge */ true)` on the `lib-jitsi-meet` path (note: `sendEndpointMessage` is `@deprecated` in favour of it, though Jitsi's own remote control still calls the old one), or `sendEndpointTextMessage` through the iframe External API, which is **string-only** and therefore JSON-stringified. Received via `ENDPOINT_MESSAGE_RECEIVED` on both. Messages are **unicast to the sharer** rather than broadcast — only the sharer renders, so the capture path does the fan-out and bridge load stays flat as the room grows. The envelope mirrors Jitsi's own convention for exactly this class of feature (`{ name: 'casual-annotate', ...op }`, the shape remote control uses at `functions.ts:43`), so a room carrying both demultiplexes cleanly. This is what makes **browser annotators work**. `iroh-gossip` (ADR-094, already in the tree at `0.101`) is the deferred second adapter behind the same interface.
  - **Decision (3) — split the stroke, and make undo a tombstone.** RAS sends one message per *completed* stroke (`AnnotateOp::Stroke`); the SDK sends `begin` / `append` (≤64 pts) / `end`, which keeps every message far under the relay's practical size bound and gives live drawing instead of appear-on-pen-up. Every op carries an author and a **stroke id**, and `undo` **tombstones by id** — never `annotStrokes.pop()` (`overlay.js:68`), which with several authors on a lossy relay deletes someone else's work. Tool tags (`0=pen,1=highlighter,2=arrow,3=rect`) and normalized `0..=65535` coords are **unchanged from `ras-protocol`**, so the deferred iroh path stays wire-compatible and the `overlay.js` geometry ports untranslated.
  - **Decision (4) — one colour per participant, assigned by the sharer; removes are author-scoped; cursors are named.** Author = the conference endpoint id. **Colour is assigned, not hashed** — a deterministic hash collides, and the moment two participants share a colour "erase my colour" stops meaning anything to a human watching the screen, and one user's eraser appears to eat another's work; the sharer is already the sole renderer, the authority and the roster holder, so it is the only place a collision-free assignment exists. It hands out palette slots on join, releases them on leave, and broadcasts a `roster` op (the one place the sharer talks *back* to annotators). **Colour therefore leaves the wire entirely** — the sharer derives a stroke's colour from its author — which drops a field and closes a spoofing vector: a participant cannot draw in someone else's colour because they never state one. The user never picks a colour, so the RAS `.swatch` picker (`main.js`) is deleted. **`undo`, `erase` and `clear: "mine"` are scoped to the relay-reported sender**, enforced sharer-side by filtering ids by author rather than trusting them, and keyed internally on **author id, never colour**, so removes stay exact even under palette exhaustion. The eraser hit-tests locally, which obliges the annotator to retain its own strokes' geometry after the visual echo fades. **Ops are attributed to the relay-reported sender, never to a `from` field inside the payload** (there is none), so a participant cannot draw as, or erase, someone else. N live cursors are rendered on the sharer's overlay, each carrying its participant's **display name** in that participant's colour, taken from the sharer's roster. This reverses ADR-100's removal of the look-here pointer *and the reversal is the point*: ADR-100 removed it because in **1:1** a single unlabelled arrow beside the host's own baked-in cursor read as a confusing second cursor (reported on Linux); in a multi-party meeting several pointers, each carrying a **display name** in that participant's colour, are precisely the feature, and the name + colour are exactly what stop them reading as the system cursor. ADR-100's click-through/transparency rule is **not** reversed.
  - **Verified against the Jitsi source, not assumed (all three repos are Apache-2.0 — clean against ADR-051's `deny.toml` allow-list).** Three findings shaped the design. (a) **Our coordinate mapping is better than the reference implementation's:** Jitsi remote control normalizes against the large-video **wrapper element** (`actions.ts:637` over `VideoLayout.getLargeVideoWrapper()`), which skews under any letterboxing; RAS's `videoContentRect()` mapping (`main.js:2075`) is letterbox-aware, so we port **ours**. (b) **`sourceId` → display is already solved and is ugly** — `@jitsi/electron-sdk`'s `RemoteControlMain._getDisplay()` needs a native addon on Windows, parses `screen:<displayId>:0` on macOS, must force `scaleFactor` to 1 on macOS (bounds already include it), re-resolves on `display-metrics-changed`, and **returns `undefined` on Linux with more than one display**. We reuse that logic rather than rediscover it, and inherit its limit: **Linux multi-monitor is unsupported**, so annotation disables itself there rather than painting on the wrong screen. (c) **The renderer-relays → main-process-owns-the-privileged-work architecture is Jitsi's own** (`remotecontrol/{renderer,main}.js`); we copy its shape and its native consent gate, minus `robotjs` and the entire input-injection path.
  - **Security posture.** Annotation remains **display data — geometry and a colour — with no OS input, screen-write, or filesystem path**, so ADR-097's "**no capability required**" reasoning carries over, and that is exactly what makes it safe to drop into a third-party meeting product. It is **sharer-authoritative** (Inv 1): off by default per share, an always-visible indicator on the always-on-top overlay naming who is drawing (Inv 7, reusing the RAS badge), instant clear+revoke, per-participant mute, and an allow-list — any room participant can *send*, so the sharer decides who is *honoured*. Bounds carry over verbatim and **fail closed**: `MAX_ANNOT_POINTS = 1024`, 256 retained strokes, plus per-author live-stroke and rate limits; unknown `op`/`tool` tags are **rejected, not clamped**. Marks are content and are **never logged** (Inv 8). **Stated honestly, not papered over:** in a standard meeting the bridge relays ops in the clear — the same trust the meeting already places in it for media — but **with Jitsi E2EE enabled, media is end-to-end encrypted and endpoint messages are not**, so annotation geometry would be visible to the bridge when the video is not. That gap is the strongest argument for the deferred iroh transport.
  - **Decision (5) — treat the ~1 s draw-to-see delay as a configuration bug first, and never fade the local echo on a fixed timer.** On-device the delay is the feature's biggest risk, and the source says most of it is Jitsi's default, not our architecture: `SS_DEFAULT_FRAME_RATE = 5` (`ScreenObtainer.ts:125`, `config.js:396`) sets screen share to **5 fps**, which simultaneously (i) puts a 200 ms capture tick in the path and makes ink arrive in visible *steps*, (ii) selects `contentHint = 'detail'` — spatial quality over temporal smoothness, backwards for a moving pen — because the test is literally `max > 5 ? 'motion' : 'detail'` (`ScreenObtainer.ts:422`), and (iii) caps screenshare bitrate / disables simulcast lower layers via `maxFps <= 5` (`TraceablePeerConnection.ts:2281`). **Raising `desktopSharingFrameRate.max` above 5 while annotation is active flips all three with one lever**, at a documented cost in resolution (`config.js:397`) — hence only while annotation is on, restored afterwards. Second: an earlier draft of this ADR specified a **fixed ~1 s cross-fade** of the local echo, which is **wrong and is itself the artifact** — when the round trip is also ~1 s the local mark vanishes exactly as the video copy arrives, so it blinks. The echo instead holds until the sharer **`ack`s** the stroke over the data channel (a data RTT, not a video round trip), then holds for the estimated video lag, then fades over ~400 ms, and never fades while the pointer is down; a lingering ghost is strictly better than a gap. The estimate can later be replaced by a real measurement — a low-visibility **pixel beacon** drawn in the overlay corner and sampled from the incoming video with `drawImage` gives true end-to-end video latency.
  - **Trade-off / considered + rejected.** Accepted costs: (a) **the delay is irreducible for passive viewers** — the annotator has its echo and the sharer has no video round trip at all, but everyone else waits out the full path, and §11.1 is their only lever. The fix that would cover them — broadcasting ops and having every viewer render a local low-latency preview over the large video, with the sharer's desktop still authoritative — is **deferred, not rejected**: it drags in the whole broadcast + per-viewer-mapping + tile-view-refusal machinery of the fallback, and raising the frame rate may make it unnecessary; (b) **window shares don't work** — an overlay is not captured when the sharer shares a *window* rather than a display, so v1 **detects the source kind and says so** rather than silently drawing nothing; (c) **a browser sharer cannot host an overlay at all**. Considered + rejected: **rendering on every participant's video element** as the primary model — it needs broadcast instead of unicast, needs real late-joiner state sync, and puts marks per-viewer instead of in the shared image; kept only as the **fallback** for browser sharers and window shares (`main.js:2022` is already that renderer). Considered + rejected for v1: **a Rust core + N-API addon** — the data path is a browser/Electron data channel and the renderer is a canvas, so Rust would be overhead on the critical path; **v1 is TypeScript**, with the op schema mirrored into `ras-protocol` and Rust re-entering with the iroh transport.
  - **Decision (6) — if the delay survives, decompose it before offloading anything; and B2 is a trap.** Offloading the screen share off JVB is the obvious next move if §11.1 is not enough, but "~1 s" is three numbers and each points at a different fix, so A2.5 splits them: **t1** draw→sharer (the `ack`), **t2** sharer-renders→encoded-outgoing-frame (pixel beacon on the local preview track), **t3** outgoing-frame→viewer-displays (beacon at the viewer, minus t2). t2 dominating is capture/encode and is **cheap**; t3 dominating is JVB relay + jitter buffer + decode and is the **only** case where bypassing the media path helps; t1 dominating means the answer is the transport (ADR-094/§14), not a media server. **Two ~2-line levers are exhausted first:** `desktopSharingFrameRate.max > 5` against t2, and **`receiver.playoutDelayHint = 0`** against t3 — verified **unused anywhere in `lib-jitsi-meet`**, though the `playout-delay` RTP header extension is in its SDP and `findReceiverForTrack` (`TraceablePeerConnection.ts:2365`) already yields the receiver. Only then, the shapes: **B1** (own capture in Electron, `replaceTrack` into the Jitsi peerconnection) fixes t2 with no server; **B2** (sharer → our server → a headless client republishing into the room) is **explicitly rejected and named so it is not built by accident** — viewers still receive through JVB so t3 is untouched, and it adds a hop, making the measured problem *worse*; **B3** (our stack carries screen + annotation to every participant, Jitsi keeps people and audio) is the only shape that fixes t3, and it is **a different product, not a latency fix** — RAS screen sharing is 1-host→1-controller, so N-way is exactly the "iroh doesn't do groups" problem, and `iroh-gossip` does **not** solve it (a ≤4 KiB unordered signalling overlay is not a video fanout; B3 needs media fanout that does not exist in this repo). B3 may well be where the SDK thesis points, but it earns its own roadmap slot on its own merits.
  - **Decision (7) — mobile is a first-class participant, so compatibility is a wire property and the media path stays on Jitsi by default.** Web and Electron ship on demand; a mobile app ships when a store review and then a user's update habits allow, so **the deployed population is permanently mixed** and a synchronised-upgrade assumption breaks mobile. §2 rescues the common case: **viewing annotations needs no client code at all** — the marks are in the screen-share video, so every participant, including a years-old mobile build, sees them correctly and always will. Only *drawing* needs our code, which narrows compatibility to one direction: a current sharer must keep understanding an old annotator's ops. Four rules, implemented in `core/compat.js` and enforced by tests: **(R1)** accept a **version range**, never a single version — the original `v !== PROTOCOL_VERSION` check in this ADR's first implementation would have cut off every un-updated client; **(R2)** **never add a required field** — new information is optional and its **absence means the old behaviour**; **(R3)** unknown **op tags are ignored** (a newer peer's feature, benign) while malformed **known** ops remain hard rejections — `decode` returns `kind: 'ignore' | 'reject'` precisely so forward-compatibility is never mistaken for an attack; **(R4)** capabilities are advertised (`hello` from annotators, `caps` on `roster` from the sharer, **both optional in both directions**, absent ⇒ `BASELINE_CAPS`) and senders gate UI on them, so a new client never offers a button whose ops the sharer would silently drop. Mobile can **see** annotations with no code and **draw** with the SDK, but can never **host the overlay** (no transparent always-on-top desktop window) or use an offloaded media path (no native QUIC). **This settles Decision (6)'s open question: B0 — Jitsi's own media path — is the default and stays the default.** Offload becomes *configurable and capability-negotiated*, never assumed: a meeting will always contain a client that cannot do it, so a mixed meeting means the sharer publishes **twice**, which prices **B3 materially higher than Decision (6) estimated** — it is not replacing the media path but running a second one alongside the one you must keep anyway. The roadmap consequence is that **§11.1's frame-rate lever is worth far more than it first looked**: it improves latency for *every* client, mobile included, with a config change and no release cycle.
  - **Decision (8) — the SDK runs INSIDE the meeting page; an out-of-iframe host cannot receive.** Found the expensive way, after the feature had been verified in pieces and still could not work end to end: `notifyEndpointTextMessageReceived` is **defined at `API.js:1700` and has zero callers in jitsi-meet's web app** (only `mobile/external-api/middleware.ts:232` implements the equivalent), so the iframe External API's `endpointTextMessageReceived` event **never fires on web or Electron**. A host outside the iframe can send into a conference and never hear back, which makes an out-of-iframe transport structurally **one-way** and the ask-and-approve flow **impossible** — a request reaches nobody, so no prompt can ever appear, and the symptom is a participant clicking "Request to annotate" and waiting forever while the sharer sees nothing and no error is raised anywhere. **The SDK therefore ships as a script served by jitsi-meet itself through its own `body.html` include (`index.html:214`)**, running in the page with the real `lib-jitsi-meet` conference, which does receive. The transport, the sharer session and the consent prompt all live there; **only the overlay window** — an OS-level always-on-top surface a browser cannot have — stays in the Electron main process, fed ops over `postMessage` → IPC. A secondary benefit is that there is now **one transport**, the same code in a browser and in the desktop app, so the two halves cannot drift apart (they had: `ConferenceTransport` put `{name:'casual-annotate'}` straight on the bridge while `ExternalApiTransport` only ever surfaced `endpoint-text-message`, so browser→Electron traffic arrived and was silently dropped).
  - **Decision (9) — a grant must be impossible without a request, not merely intended.** A live run produced permission **granted with no prompt ever shown** — strictly worse than having no consent feature, because the interface claims a gate that is not there. The cause was that "only called after a human decision" was a **comment**, and a comment is not an enforcement: any stray call, replayed state update, or second session object in the same page could grant silently. It is now structural: `allowParticipant` **refuses** unless that participant has a request outstanding; `preAuthorize` is the sole path to admitting someone who never asked and is deliberately verbose so every such grant is a visible line in the host application; `approve` returns `false` and emits nothing when there was nothing to approve. A withdrawn permission cannot be restored by a replayed approval, because the original request is spent. Revocation additionally **resets the annotator's tool to `null`** — re-rendering the toolbar alone left the canvas accepting pointer events and emitting ops the sharer now refuses, so the user drew into a void with no feedback. Pinned by tests named after the defect.
  - **Decision (10) — the Electron desktop adapter was never migrated after Decision 8, and that is the actual reason the consent loop had never completed; fix it by injecting the working transport into the iframe, not by rebuilding consent.** Auditing the branch after Decision 9 turned up a defect Decision 8/9 did not close: `adapters/jitsi-electron/{main,renderer}.js` — the real jitsi-meet-electron wiring, and the one path with a **native** OS consent dialog — still ran the sharer/annotator session over `ExternalApiTransport`, whose receive path is exactly the dead `endpointTextMessageReceived` event Decision 8 identified, and nothing anywhere listened for the `postMessage` the in-page fix (`standalone/inject.js`) sends toward a host. So a request from an annotator could reach the Electron app and be silently dropped forever, and the native `dialog.showMessageBox` prompt it fed could never fire — confirmed by `git show` on the Decision-8 commit touching only `standalone/inject.js`, and by grepping the adapter for any `postMessage`/`__casualAnnotate` listener (none existed). **Chose to keep the native dialog and fix its transport, not replace the dialog with the in-page toast**: a native OS-level prompt is harder for a hostile page to imitate, and the app already has the plumbing for it. The fix runs on the *other* side of the iframe boundary — `main.js` now injects a small bundled relay (`injected-relay.js`) directly into the Jitsi iframe via Electron's privileged `webContents.mainFrame` `executeJavaScript`, a capability only a host application has and a real cross-origin page could never grant itself; that script holds the real conference and runs a genuine `ConferenceTransport`, where the receive event actually fires, bridging it out over `postMessage` (which, unlike direct DOM access, crosses a cross-origin iframe boundary fine). `PostMessageTransport` (`transport/jitsi.js`) is the host side, replacing `ExternalApiTransport` in the renderer — `ExternalApiTransport` itself is kept, documented as send-only, for a host that genuinely only ever sends. A useful side effect: this makes the desktop app work against **any** jitsi-meet server, not only ones an operator has edited `body.html` on — the browser deployment (§5.2, unchanged) still needs that, but the Electron path no longer does. **The session-ownership design was never wrong and is untouched**: the overlay window already held the one authoritative `SharerController` (`overlay-page.js`), fed by an ops relay — exactly the "ops, not state" shape `main.js`'s own header comment always described; only the transport feeding that relay was broken. Also closed in the same pass: an Invariant-7 gap where the "always-visible annotation indicator" was **claimed in this ADR and the design doc but never actually built** anywhere in the tree — added to the Electron overlay canvas itself (inside the captured pixels, so every remote viewer sees it, not just the sharer) and, for a browser sharer with no overlay window to draw into (§13), a local-only equivalent; and a from-the-console-only gap for muting/revoking an individual annotator, now a small panel in each host surface. The browser-facing consent card also moved from a full-screen dimming modal to a non-blocking corner toast (industry-standard shape for this class of in-meeting request — Zoom/Teams/Meet all use one), with an explicit, visible auto-deny timeout rather than silence — §9.1's "a refusal is sent explicitly" rule extended to the timeout case, which the original modal did not have at all.
  - **Status. Proposed (pending sign-off). Verified live, 2026-09-22 — the first time the full
    request → prompt → approve → draw cycle has ever been observed working, in either shape.**
    Design: `docs/design/annotate-sdk-jitsi-design.md`; deployment: `sdk/annotate/DEPLOYMENT.md`;
    Electron wiring: `sdk/annotate/adapters/jitsi-electron/README.md`; scripted live proof:
    `sdk/annotate/test/live/README.md`. 134 unit tests green; all 6 Electron bundle boundaries hold
    (`verify-bundles.mjs`, now including the injected relay). **Verified live, driven through the
    real UI (not console commands) against a real local Jitsi server (`test/live/e2e.spec.mjs` /
    `electron.spec.mjs`):**
    - **Browser → browser:** request → the sharer's toast → Allow → a real pointer-drawn stroke
      lands with correct author/colour attribution → Undo → a forged cross-author erase from an
      unadmitted sender is refused (`not-admitted`) → revoke via the sharer's management panel (not
      the console) both drops the withdrawn participant's existing marks and stops further drawing.
    - **Browser → Electron desktop (Decision 10's actual subject):** run via a hermetic harness
      (`test/electron/e2e-harness/`) wiring the real `adapters/jitsi-electron/main.js` — no jitsi-meet-
      electron checkout — the request reaches the **native `dialog.showMessageBox` seam**
      (previously provably unreachable — Decision 8/9) and the resulting stroke lands in the
      **overlay window's own rendered pixels** (capture-verified, mirroring `test/electron/smoke.mjs`'s
      technique), not just the session's stroke count.
    - Both suites pass reliably in isolation; run back-to-back in this constrained sandbox they can
      need Playwright's one configured retry (real local-Jitsi XMPP-join timing under contention, not
      a logic fault — confirmed by re-running the "failed" suite alone immediately after, clean pass).
    - A **live adversarial multi-agent review** of the whole Decision-10 change (transport, security,
      UX/a11y, state-consistency, test-quality — 5 independent review passes, each finding verified by
      a separate skeptic pass before being counted) found **23 real, confirmed defects**, since fixed:
      most seriously, `adapters/jitsi-electron/main.js`'s overlay-readiness flag was reset on every
      `CH.STOP` but only ever set once per app lifetime, so annotation silently died after any
      stop-and-reshare (an entirely ordinary flow) with zero error anywhere — and neither
      `PostMessageTransport` nor `injected-relay.js` checked `event.source` on incoming `postMessage`,
      so anything able to post into the relevant window could forge a sender id or a moderator-flagged
      roster entry. Also fixed in the same pass: a real `AnnotationSurface` crash (an infinite
      exception loop) whenever no real `<video>` element exists yet, a `startsWith`-based
      `serverOrigin` match bypassable with a suffixed hostname, a `SharerPanel` that rebuilt its whole
      DOM (dropping keyboard focus) on every cursor move — up to ~20 Hz — a toolbar rebuild that
      dropped focus on every tool selection, a consent toast that stole focus out of an active text
      field and never restored it after closing, an Escape-to-deny handler scoped to the whole
      document instead of the toast itself, and the `muted` flag in `SharerController.state()`
      conflating "never granted" with "granted, then muted" (the panel's Unmute button was
      meaningless for someone never granted). Full list and verification reasoning: the branch's own
      commit history for this pass.
    - **Not yet re-verified:** a full jitsi-meet-electron checkout (the hermetic harness wires the
      same production `main.js`/`preload.js`/`injected-relay.js`, deliberately, to avoid needing one —
      `adapters/jitsi-electron/install.mjs`'s NEW edits for Decision 10 — the relay build target, the
      `relayBundlePath`/`serverOrigin` options — have not been round-tripped against a live checkout
      the way `install.mjs`'s original edits were in the commit that added it). A2.5 (the latency
      measurement) remains undone.
    Does not touch the shipping RAS app or any existing wire format — the SDK is additive, and the

...
- **ADR-107 · Annotation extracted as an embeddable SDK; Jitsi first, over the bridge relay, rendered on the sharer's real desktop · Proposed (pending sign-off).**
  ...
  - **Status. Proposed (pending sign-off).** Design: `docs/design/annotate-sdk-jitsi-design.md`; deployment: `sdk/annotate/DEPLOYMENT.md`. Implementation under `sdk/annotate/` — 125 unit tests green. **Verified:** the overlay renders in Electron and is present inside a live screen-capture stream (the §2 claim the architecture rests on); ops cross a real JVB relay; author-scoped removes hold against hostile ops. **Not verified end to end:** a complete request → prompt → approve → draw cycle has never been observed in one run. Does not touch the shipping RAS app or any existing wire format — the SDK is additive, and the `ras-protocol` mirror is a schema-compat obligation, not a change. The phase that can invalidate the design is **A2** (one annotator → one Electron sharer, proving the capture-path fan-out end to end); it is deliberately sequenced before the surface work is polished. Depends on ADR-097 (op model) + ADR-100 (overlay discipline); relates to ADR-094 (the deferred gossip transport) and ADR-057 (the browser-controller track).

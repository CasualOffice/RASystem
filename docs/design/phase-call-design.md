...
| L6a | **App seams + commands** — implement the four seams (iroh control, `ras-signal` invite, `ras-mic`/`ras-camera` pumps, Tauri event bridge) + call Tauri commands | `app` | ⚠️ on-device | pending |
| L6b | **App UI** — wire the shell ring/incall/PiP surfaces to the call commands + lifecycle events | `app/ui` | ◐ node-check / device | pending |

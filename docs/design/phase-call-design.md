...
| L5t | **Symmetric call transport (ADR-106)** — the `CallTransport` seam + an in-memory **crosswise loopback** (`testkit::call_loopback`) proving audio/video/control flow **both ways**, off-device. The concrete iroh `casual-ras/call/1` impl is the on-device follow-up | `ras-core` (seam+loopback); `ras-transport-iroh` (iroh impl) | ◐ loopback ✓ / iroh device | **seam + loopback landed** |

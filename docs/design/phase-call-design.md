...
| L4c | **Async driver** — `ras-core`/app glue: perform `CallAction`s (send `ControlMsg`, ring, spin mic/camera pumps) over the real transport; per-message mic/camera gate at the send boundary | `ras-core`, `app` | ◐ loopback / device | pending |

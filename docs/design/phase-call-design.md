...
| L4b | **Call runtime** — pure `CallRuntime` mapping call events → wire messages + media directives + content-free lifecycle events (`CallAction`/`CallLifecycleEvent`); the driver performs the I/O | `ras-call` | ✅ unit | **landed** |
| L4c | **Call driver** — `ras-core::call::CallDriver` owns `CallRuntime` + dispatches `CallAction`s to DI seams (control sink / signal sink / media controller / event sink); transport- & media-agnostic, loopback-tested | `ras-core` | ✅ loopback | **landed** |
| L5t | **Symmetric call transport (ADR-106)** — the `CallTransport` seam + crosswise loopback (`testkit::call_loopback`) + the **iroh impl** `IrohCallTransport` (over `casual-ras/call/1`, `Session::call_planes` runs the per-frame-video + audio-datagram primitives both ways). Loopback + driver-over-transport capstone proven off-device; the true two-endpoint call is on-device | `ras-core`, `ras-transport-iroh` | ◐ loopback ✓ / iroh compile ✓ / two-machine device | **seam + loopback + iroh impl landed** |
| L6a | **App seams + commands** — implement the four seams over the call transport (`CallTransport` control, `ras-signal` invite, `ras-mic`/`ras-camera` pumps, Tauri event bridge) + call Tauri commands. **Blocked on L5t** | `app` | ⚠️ on-device | pending |
| L6b | **App UI** — wire the shell ring/incall/PiP surfaces to the call commands + lifecycle events | `app/ui` | ◐ node-check / device | **landed (node-checked)** |

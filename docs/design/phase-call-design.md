...
| L5t | **Symmetric call transport (ADR-106)** — a `CallTransport` over a dedicated `casual-ras/call/1` ALPN reusing the audio-datagram + per-frame-video + bidi-control primitives run **both ways**; the prerequisite for a *working* call. Design gate: **ADR-106 (Proposed)** | `ras-transport-iroh`, `ras-core` | ◐ loopback / device | design pending |

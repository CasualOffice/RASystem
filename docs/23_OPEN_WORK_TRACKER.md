...
| 2.2 | **Multi-monitor selection + cursor position** | `CODE-NOW`→`DEVICE` | ◐ | DONE (off-device) on macOS: real `enumerate_displays`/`captured_display`, a Share-view picker (`list_displays`/`select_display`), `HostSessionConfig` threaded with the real choice. The wire's `display_id:0` + `set_display_bounds(0,...)` convention was verified CORRECT as-is (a single-display-per-session id, not the real `MonitorId` — `CaptureGeometry` already carries the real geometry downstream). Linux/Windows fall back to an honest single "Display 1" (scap has no per-display geometry API). On-device: a real secondary-monitor pick + cursor-position check. |

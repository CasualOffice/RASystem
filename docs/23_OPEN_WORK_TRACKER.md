...
| P2 | No concurrency/rate cap on inbound SIGNAL/GOSSIP dials (task-per-dial, unbounded); `presence_poll_loop`'s `last` map accretes dead entries across churn | ◐ CAP FIXED — a shared `Semaphore` (cap 64, non-blocking `try_acquire_owned`, shed-under-flood) now bounds concurrent in-flight signal/gossip handler tasks; far above legitimate use (bounded by contact count) so normal operation never touches it. The `last`-map dead-entry prune is still open (`CODE-NOW`, trivial, cosmetic). |

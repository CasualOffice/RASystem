...
| **Clipboard sync** | ◐ spine (ADR-076) | All; TeamViewer most granular | **Medium** | **P1** | wire+gate landed (text, per-direction cap, redacted, no-auto-paste); OS backend + app pending |
...
- **Status: SECURITY SPINE LANDED (ADR-076).** `ControlMsg::ClipboardText` on the wire with the payload in a `Redacted` newtype (Inv 8 — `Debug` prints only a byte count; a secret can't leak through a derived log line), bounded by `MAX_CLIPBOARD_BYTES=768 KiB` (**refused, never truncated**), fail-closed codec + roundtrip/oversize/redaction/fuzz tests. Per-direction host-side gate `ras_policy::clipboard_push_allowed` (controller→host = `clipboard.write`, host→controller = `clipboard.read`), both **recognized-but-withheld → default OFF** (tested). **Follow-up (GUI/on-device):** the per-OS `ClipboardBackend` (NSPasteboard / X11 selections + `wl-clipboard` / Win32), `ras-core` host-loop wiring (receive → gate → set clipboard **without** paste), the app "Send clipboard" button + "clipboard shared" indicator, echo-suppression tag, pre-connection-never-synced.

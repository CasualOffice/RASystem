...
|---|---|---|---|---|---|
| **Audio forwarding** | ◐ cap+seam (ADR-077) | RustDesk/Parsec Opus; AnyDesk on-by-default | **Low** | **P1** | `audio.listen` cap + media traits landed; Opus codec + OS capture + transport pending |
...
- **Status: CAPABILITY + MEDIA SEAM LANDED (ADR-077).** `audio.listen` capability (host→controller output audio; **recognized-but-withheld → default OFF**, tested) — this is the concrete name for the proposed `audio.system.play`; the separate future mic cap stays `audio.mic.capture`. `ras-media::audio` defines the pipeline as traits + canonical types (`AudioConfig`, `CapturedAudio` interleaved-i16 PCM, `EncodedAudio` = one Opus packet with monotonic `seq`, **no keyframes**; `AudioCaptureBackend`/`AudioEncoderBackend`/`AudioDecoderBackend`), parallel to the video traits, with dependency-free `SyntheticAudioCapture`+`SyntheticAudioEncoder` doubles + a capture→encode roundtrip test. **Follow-up (OS/on-device):** real Opus (`opus`/libopus BSD-3, own ADR), OS output-audio capture (SCK-audio / WASAPI-loopback / PipeWire), the audio QUIC sub-stream + `AudioConfig` negotiation, `ras-core` pump + `audio.listen` gate + "AUDIO SHARED" Inv-7 indicator, JS `AudioDecoder`→`AudioContext` playback. Live-only, never recorded (Inv 12).

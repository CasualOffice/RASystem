...
- **Status: wire + fail-closed codec LANDED (ADR-073).** The three `ControlMsg` variants ship with bounds (`MAX_CURSOR_DIM=256`, exact `w*h*4` RGBA length, hot-spot-inside, zero-dim reject), by-example + property + fuzz tests. Host cursor **capture** + controller **render** are the on-device/GUI follow-up; cursor *position* sync is a later addition (control mode composites at the controller's own pointer).

...
mod e2e {
    ...
    #[tokio::test]
    async fn controller_annotation_reaches_host_as_a_lifecycle_event() {
        ...
        // The viewer draws a two-point arrow. Annotation is display data (no capability), like the
        // pointer — it must surface on the host as a RemoteAnnotation lifecycle event for the overlay.
        controller.send_annotation(AnnotateOp::Stroke {
        ...
        assert_eq!(
            ...
            "host should receive the controller's annotation stroke intact"
        );

...
//!
//! Where `ras-cursor-macos` reports only shape changes, this observer — living next to the
//! ScreenCaptureKit backend — also tracks the pointer's **position** so the controller can render a
//! zero-latency remote cursor that both looks right (shape) and lands right (position):
//!
